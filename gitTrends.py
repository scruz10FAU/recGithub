...
def transform(repos_html):
    result=[]
    for r in repos_html:
        number_of_stars = ''.join(r.select_one('a.Link.Link--muted.d-inline-block.mr-3').text.split())
        stars_today = ''.join(r.select_one('span.float-sm-right').text.split())
        stars_today = number_of_stars.replace("starstoday", "")
        ...
        result.append({'Developer': developer_name, 'Repository Name': repository_name, 'Number of Stars': number_of_stars, 'Stars Today': stars_today, 'Language': language})
    return result

def format(repositories_data):
    columns = ['Developer, Repository_ame, Number of Stars', 'Stars Today', 'Language']
    for repository in repositories_data:
        r = [repository['Developer'], repository['Repository Name'], repository['Number of Stars'], repository['Stars Today'], repository['Language']]
